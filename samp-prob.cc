 ...
 * samp-prob.cc
 *
 * This program performs probabilistic sampling.
 *
...
int tsSec() {
  ...
  int line_num = 0; // line ID for printing
	...
	// sampling controls
	unsigned int totsampled = 0, rnum = 0, totpkt = 0;
	srand(time(NULL));
...
int main(int argc, char **argv) {
  ...
  if (argc < 4) {
    fprintf(stderr, "Usage:\n# %s <P> <pcap_file> <dump_file>\n", argv[0]);
    exit(1);
  }

	read_controls.p = atoi(argv[1]);
